...
function Xajax() {
  ...
  this.getInput = function (sType, sName, sId) {
    ...
    if (!window.addEventListener) {
      Obj = document.createElement(
        '<input type="' + sType + '" id="' + sId + '" name="' + sName + '">'
      );
    } else {
            ...
            for (var j = 0; j < formElements[i].length; j++) {
              if (formElements[i].options[j].selected == true)
                sXml +=
                  name +
                  "=" +
                  encodeURIComponent(formElements[i].options[j].value) +
                  "&";
            }
          } else {
  ...
  this.call = function (sFunction, aArgs, sRequestType) {
    ...
    r.onreadystatechange = function () {
      ...
      if (r.status == 200) {
        ...
        if (xajax.arrayContainsValue(xajax.responseErrorsForAlert, r.status)) {
          var errorString =
            "Error: the server returned the following HTTP status: " + r.status;
          errorString += "\nReceived:\n" + r.responseText;
          alert(errorString);
        }
        document.body.style.cursor = "default";
    ...
    if (xajaxDebug)
      this.DebugMessage(
        "Calling " + sFunction + " uri=" + uri + " (post:" + postData + ")"
      );
    r.send(postData);
      ...
      if (xml.childNodes[i].nodeName == "cmd") {
        ...
        if (
          ...
          (xml.childNodes[i].firstChild.nodeName == "#cdata-section" ||
            xml.childNodes[i].firstChild.nodeName == "#text")
        ) {
          data = "";
          ...
          for (var j = 0; j < xml.childNodes[i].childNodes.length; j++) {
            if (
              ...
              (xml.childNodes[i].childNodes[j].firstChild.nodeName ==
                "#cdata-section" ||
                xml.childNodes[i].childNodes[j].firstChild.nodeName == "#text")
            ) {
              var internalData = "";
              for (
                var k = 0;
                k < xml.childNodes[i].childNodes[j].childNodes.length;
                k++
              ) {
                internalData +=
                  xml.childNodes[i].childNodes[j].childNodes[k].nodeValue;
              }
            } else {
              var internalData =
        ...
        try {
          ...
          if (cmd == "al") {
            ...
            eval(
              "this.$('" + id + "')." + property + "= function(){" + data + ";}"
            );
          } else if (cmd == "ah") {
          ...
          if (xajaxDebug)
            alert(
              "While trying to '" +
                ...
                "\n" +
                (id && !objElement
                  ? "Object with id='" + id + "' wasn't found.\n"
                  : "")
            );
        }
        delete objElement;
